	...
	b、Mysql底层数据结构选型:
		1、哈希表（Hash）
			...
			a、哈希算法
				...
				缺点：
					1、由于是将key通过哈希函数转换成固定的地址，这个转换并不是对不同的数据转换以后会有不同的结果。有可能会导致
		...
		4、AVL二叉树（平衡二叉树）
			...
			问：为什么Map底层选用红黑树而不用AVL树？
				答；AVL树是完全平衡树，节点之间的高度差不会超过1，所以相应的其查找的速度会优于红黑树
					但是，因为其是完全平衡树，所以为了保持平衡，付出的维持平衡的时间会相应的增多，所以，对于插入
				和修改是劣于红黑树的。
					而Map集合显然更注重于数据的插入，所以选择了红黑树。
					并且，在集合中常用于线程安全的concurrentMap。在插入数据时，由于线程安全，有了锁机制，所以相应
				的插入数据时，等待的时间也会变长，也不友好。
					所以，两者比较之中，红黑树比AVL树更适合。
