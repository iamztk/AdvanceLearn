...
对象（该对象目前只是创建出来了，即new出来了，相应的属性还没有注入，在这一步注入）
	...
	当A进行创建时，在注入阶段，
	先将经过反射获取的A对象，存于二级缓存中，然后再去注入，注入时发现需要依赖B，则B去创建bean
	当B在创建bean时，再注入阶段，将经过反射创建的B对象存进缓存中，然后再去注入A，此时发现需要依赖A，则去创建A，
	此时发现A能从缓存中获取，直接从缓存中获取
	
	然后B注入A，完成后发现B中注入的是代理类？？ 是从哪里来的？
		在B注入A时，注入的就已经是代理类了。
		因为存放的是工厂，所以工厂可以对这个bean对象进行加工
		当从工厂中得到对象时，就有可能是加工后的对象了。
		例如，当对象需要进行aop时，从工厂中拿到的对象就已经是代理对象了。
