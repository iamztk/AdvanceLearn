...
1、IOC的概念和原理
	...
	扩展:
		1、通过引入IOC容器，利用依赖关系注入的方式，实现对象之间的解耦。
		2、既然IOC是控制反转，那么到底是“哪些方面的控制被反转了呢？
			答：经过详细地分析和论证后，得出了答案：“获得依赖对象的过程被反转了”。
				控制被反转之后，获得依赖对象的过程由自身管理变为了由IOC容器主动注入。
		3、IoC 不是一种技术，只是一种思想，一个重要的面向对象编程的法则，它能指导我们如何设计出松耦合、更优良的程序。
		4、DI:依赖注入
			谁依赖于谁：当然是应用程序依赖于IoC容器；
		　　为什么需要依赖：应用程序需要IoC容器来提供对象需要的外部资源；
		　　谁注入谁：很明显是IoC容器注入应用程序某个对象，应用程序依赖的对象；
		　　注入了什么：就是注入某个对象所需要的外部资源（包括对象、资源、常量数据）。
		5、IoC和DI由什么关系呢？其实它们是同一个概念的不同角度描述。
			“依赖注入”，相对IoC 而言，“依赖注入”明确描述了“被注入对象依赖IoC容器配置依赖对象”。
		6、Spring所倡导的开发方式就是如此，所有的类都会在spring容器中登记，告诉spring你是个什么东西，你需要什么东西，
			然后spring会在系统运行到适当的时候，把你要的东西主动给你，同时也把你交给其他需要你的东西。
			所有的类的创建、销毁都由 spring来控制，也就是说控制对象生存周期的不再是引用它的对象，而是spring。
			对于某个具体的对象而言，以前是它控制其他对象，现在是所有对象都被spring控制，所以这叫控制反转。
	
		7、FactoryBean 和 BeanFactory的区别？
			a、FactoryBean：是一个bean，其内有getObject()方法，可以修改实际返回的对象。类似于代理模式：
				功能：用来代理一个对象，对该对象的所有方法做一个拦截
				
			b、BeanFactory：顾名思义，这是一个工厂类，用来管理bean的工厂。
...
10、bean的生命周期
	...
	3、bean的生命周期（7步）
		在第四步的前后还有各一步，为调用spring-bean的前后置方法。
		--调用bean的前置方放弃啊
		bean可以使用了（对象获取到了）
		--调用bean 的后置方法。
		这两步需要配置才能生效，配置如下：
